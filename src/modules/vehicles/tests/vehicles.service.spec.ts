...
describe("VehiclesService", () => {
  ...
  describe("findVehicleStateAtTimestamp", () => {
    it("should return vehicle state at timestamp", async () => {
      // Arrange
      jest
        .spyOn(stateLogsService, "findStateAtTimestamp")
        .mockResolvedValue(mockStateLog);
    ...
    it("should throw NotFoundException if vehicle not found", async () => {
      ...
      // Act & Assert
      await expect(
        service.findVehicleStateAtTimestamp(mockVehicle.id, mockTimestamp)
      ).rejects.toThrow(
  ...
  describe("findAndValidateVehicle", () => {
    ...
    it("should throw NotFoundException if vehicle not found", async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(
        `Vehicle not found with id ${mockVehicle.id}`
      );
    });
  });
});
